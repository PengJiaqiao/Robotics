...
void MappingNode::laserReceived( const sensor_msgs::LaserScanConstPtr& laserScan ) {
    ...
    ///////////////////////////////////////////////////
	// <your code>
    //generate an 1000x1000 field (such as map size) to collect hit an miss values
    int hits[1000][1000] = {};
	int misses[1000][1000] = {};
	

    for (int i = 0; i < (laserScan->angle_max - laserScan->angle_min)/laserScan->angle_increment ; i++) //iterations through all laser measurements in 1 round
    {
    	if( (laserScan->range_min < laserScan->ranges[i]) && (laserScan->ranges[i] < laserScan->range_max) ) //if measured laser is in valid range
    	{
			vector<pair<int, int> > result;

    		//Calculate end of laser for bresenhamLine()      
    		double x_laserbeam_end = robotX + laserScan->ranges[i]/map.info.resolution * cos (robotTheta + laserScan->angle_min + laserScan->angle_increment*i);
			double y_laserbeam_end = robotY + laserScan->ranges[i]/map.info.resolution * sin (robotTheta + laserScan->angle_min + laserScan->angle_increment*i);

			result = bresenhamLine( robotX, robotY, x_laserbeam_end, y_laserbeam_end);
			double value_update = 0;

			for (int j = 0; j < result.size(); j++) // iterate through all point included in laser beam
		    {
		    	if(j == result.size()-1 ) //if we get the last point of laser beam which is where we have a hit
		    	{
		    		hits[result[j].first][result[j].second]++;
		    	}
		    	else //measurement point before the end of the laser beam - these are the missis
		    	{
		    		misses[result[j].first][result[j].second]++;
		    	}
		    	//calculate probability hit/(hit+miss)
		    	value_update = hits[result[j].first][result[j].second] / (hits[result[j].first][result[j].second]+misses[result[j].first][result[j].second]);
		    	//update reflection map value
		    	updateReflectionMapValue(result[j].first, result[j].second , value_update);				
		    }

		    //DEBUGGING
		    //ROS_INFO("%f",value_update);

    	}
    	else
    	{
    		// do nothing: Laserscan beam is not in valid range
    	}
    }
